...
def test_nut_slides_down_peg():
    """Nut centered over peg should descend the shaft and assemble."""
    env = NutAssemblyEnv(gui=False)
    env.reset(seed=42)
    ...
    peg_pose = Pose(position=(peg_cx, peg_cy, peg_top_z / 2), orientation=(0, 0, 0, 1))
    grasp_tf = multiply_poses(ee_pose.invert(), nut_pose)
    grasp_z_offset = float(ee_pose.position[2]) - float(nut_pose.position[2])
    ...
    descent_target_z = peg_top_z - grasp_z_offset - 0.015

    for _ in range(10):
        ik_step = p.calculateInverseKinematics(
            robot.robot_id,
            robot.end_effector_id,
            (peg_cx, peg_cy, descent_target_z),
            targetOrientation=down_quat,
            maxNumIterations=1000,
            residualThreshold=1e-6,
            physicsClientId=pcid,
    ...
    env.step(np.concatenate([np.zeros(7), [1.0]]).astype(np.float32))
    for _ in range(200):
    ...
    ), "Nut should be assembled after sliding down peg"
